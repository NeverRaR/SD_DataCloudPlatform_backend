...
public class MainDataController {
    ...
    public @ResponseBody
    Result<ImportDataResult> importMainData (@CookieValue(value = "sessionId",
            defaultValue = "noSession") String sessionId, @RequestBody ImportMainDataRequest body) {
        User user=authenticationService.getUser(sessionId);
        if(user==null)  {
            return Result.wrapErrorResult(new InvalidSessionIdError());
        }
        return mainDataService.importMainData(body,user);
    }
